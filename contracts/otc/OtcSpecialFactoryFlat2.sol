...
pragma solidity =0.8.17 ^0.8.0 ^0.8.17 ^0.8.8;
...
// contracts/otc/extensions/ERC20X.sol
 ...
 */
contract ERC20X is Context, IERC20, IERC20Metadata {
    // mapping(address => uint256) private _balances;
    mapping(bytes32 => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _termTimestamp;
    uint256 private _nextTermTimestamp;
    ...
    constructor(string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
        _termTimestamp = block.timestamp;
        _nextTermTimestamp = _termTimestamp + 7 days;
    }
    ...
    function _getHash(address account) private view returns (bytes32) {
        bytes32 hash;
        if (block.timestamp > _nextTermTimestamp) {
            hash = keccak256(abi.encode(account, _nextTermTimestamp));
            return hash;
        }
        hash = keccak256(abi.encode(account, _termTimestamp));
        return hash;
    }

    function _updateTimestamp() private {
        if (block.timestamp > _termTimestamp) {
            _termTimestamp = _nextTermTimestamp;
            _nextTermTimestamp = _termTimestamp + 7 days;
        }
    }

    function _syncTimestamp(uint256 _timestamp) internal virtual {
        _termTimestamp = _timestamp;
        _nextTermTimestamp = _timestamp + 7 days;
    }
    ...
    function balanceOf(
        ...
        return _balances[_getHash(account)];
    }
    ...
    function _transfer(
        ...
        uint256 fromBalance = _balances[_getHash(from)];
        require(
            fromBalance >= amount,
            "ERC20: transfer amount exceeds balance"
        );
        unchecked {
            _balances[_getHash(from)] = fromBalance - amount;
            // Overflow not possible: the sum of all balances is capped by totalSupply, and the sum is preserved by
            // decrementing then incrementing.
            _balances[_getHash(to)] += amount;
        }
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        unchecked {
            ...
            _balances[_getHash(account)] += amount;
        }
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        uint256 accountBalance = _balances[_getHash(account)];
        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
        unchecked {
            _balances[_getHash(account)] = accountBalance - amount;
...
// contracts/otc/extensions/ERC20Permit.sol
 ...
 * _Available since v3.4._
 */
abstract contract ERC20XPermit is ERC20X, IERC20Permit, EIP712 {
...
// contracts/otc/OtcTokenSpecial.sol
...
// contracts/otc/OtcSpecialFactory.sol

contract OtcTokenSpecialFactoryFlat is Ownable {
